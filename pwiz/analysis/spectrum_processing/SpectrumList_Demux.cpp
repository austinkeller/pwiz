...
namespace analysis {
    ...
    class lock_guard_semaphore
    {
    public:
        lock_guard_semaphore(boost::interprocess::interprocess_semaphore& s) :
        s_(s)
        {
            s_.wait();
        }
        ~lock_guard_semaphore()
        {
            s_.post();
        }

    private:
        boost::interprocess::interprocess_semaphore& s_;
    };
    ...
    private:
        ...
        /// This caches the last several solutions generated by the DemuxSolver.
        ...
        /// We cache more than a single spectrum to handle multi-threaded sequential access. We cache roughly one solution per possible thread.
        class SolutionCache
        {
        public:

            /// Shared pointer definition
            typedef boost::shared_ptr<SolutionCache> ptr;

            /// Constant shared pointer definition
            typedef boost::shared_ptr<const SolutionCache> const_ptr;

            SolutionCache(SpectrumListPtr sl, IDemultiplexer::const_ptr demuxer, double demuxBlockExtra, DemuxSolver::const_ptr demuxSolver, size_t maxSolutionCacheSize) :
                sl_(sl),
                demuxer_(demuxer),
                demuxBlockExtra_(demuxBlockExtra),
                demuxSolver_(demuxSolver),
                maxSolutionCacheSize_(maxSolutionCacheSize)
            {}

            MatrixPtr GetSolution(size_t originalSpectrumIndex) const;
            size_t CacheMisses() const { return cacheMissTracker_.numMisses(); }
        private:

            static void GenerateSolution(size_t originalSpectrumIndex,
                MatrixPtr& solution,
                IDemultiplexer::const_ptr demuxer,
                double demuxBlockExtra,
                DemuxSolver::const_ptr demuxSolver);

            size_t maxSolutionCacheSize_;
            IDemultiplexer::const_ptr demuxer_;
            double demuxBlockExtra_;
            DemuxSolver::const_ptr demuxSolver_;

            struct CacheEntry
            {
                MatrixPtr solution;
                boost::mutex mutex; ///< don't let more than one thread solve and duplicate effort
            };
            mutable std::map<size_t, boost::shared_ptr<CacheEntry>> solutions_;
            mutable boost::mutex solutionsMutex_;
            
            SpectrumListPtr sl_;

            // Track cache misses for debugging performance issues
            struct CacheMissTracker
            {
            public:
                CacheMissTracker() : numMisses_(0) {}
                void AddCacheIndex(size_t index) const {
                    boost::lock_guard<boost::mutex> lock(mutex_);
                    if (previouslyCachedIndices_.find(index) != previouslyCachedIndices_.end())
                        ++numMisses_;
                    else
                        previouslyCachedIndices_.insert(index);
                }
                size_t numMisses() const { boost::lock_guard<boost::mutex> lock(mutex_); return numMisses_; }
            private:
                mutable boost::mutex mutex_;
                mutable size_t numMisses_;
                mutable std::set<size_t> previouslyCachedIndices_;
            } cacheMissTracker_;
        };
        SolutionCache::ptr cachedSolutions_;

        boost::scoped_ptr<boost::interprocess::interprocess_semaphore> threadlimitSemaphore_;
    ...
    inline size_t size_diff(const size_t& lhs, const size_t& rhs)
    {
        return lhs > rhs ? lhs - rhs : rhs - lhs;
    }

    MatrixPtr SpectrumList_Demux::Impl::SolutionCache::GetSolution(size_t originalSpectrumIndex) const
    {
        boost::shared_ptr<CacheEntry> cacheEntry;
        {
            boost::lock_guard<boost::mutex> lock(solutionsMutex_);

            auto lastSolutionIt = solutions_.find(originalSpectrumIndex);
            if (lastSolutionIt == solutions_.end())
            {
                solutions_.insert({ originalSpectrumIndex, boost::make_shared<CacheEntry>() });
            }
            cacheEntry = solutions_.at(originalSpectrumIndex);
        }

        {
            // Wait if solution for this index is currently being solved
            boost::lock_guard<boost::mutex> solveLock(cacheEntry->mutex);

            if (cacheEntry->solution)
            {
            ...
            try
            {
                cacheMissTracker_.AddCacheIndex(originalSpectrumIndex);
                GenerateSolution(originalSpectrumIndex, cacheEntry->solution, demuxer_, demuxBlockExtra_, demuxSolver_);
            }
            catch (exception& e)
            {
                throw e;
            ...
            {
                cerr << "Unknown error generating solution on index " << originalSpectrumIndex << endl;
                throw;
            }
            //CacheSolution(originalSpectrumIndex, cacheEntry->solution);
        }
        ...
        try
        {
            ...
            std::vector<size_t> cachedIndices;
            for (auto p : solutions_)
            {
                cachedIndices.push_back(p.first);
            }
            std::sort(cachedIndices.begin(), cachedIndices.end(),
                [originalSpectrumIndex](size_t lhs, size_t rhs){ return size_diff(lhs, originalSpectrumIndex) > size_diff(rhs, originalSpectrumIndex); });
            auto indexIt = cachedIndices.begin();
            while (solutions_.size() > maxSolutionCacheSize_)
            {
                solutions_.erase(*indexIt);
                ++indexIt;
            }
        }
        catch (exception& e)
        ...
        {
            cerr << "Unknown error trimming cache" << endl;
            throw;
        }
    ...
    void SpectrumList_Demux::Impl::SolutionCache::GenerateSolution(size_t originalSpectrumIndex, MatrixPtr& solution,
        IDemultiplexer::const_ptr demuxer, double demuxBlockExtra,
        DemuxSolver::const_ptr demuxSolver)
    {
        if (!solution)
        {
            ...
            try
            {
                demuxer->GetMatrixBlockIndices(originalSpectrumIndex, muxIndices, demuxBlockExtra);
            }
            catch (exception& e)
            {
                throw e;
            ...
            {
                cerr << "Unknown error in GetMatrixBlockIndices on index " << originalSpectrumIndex << endl;
                throw;
            }
#ifdef _PROFILE_PERFORMANCE
            ...
            std::cout << "GetMatrixBlockIndices: " << duration << endl;
#endif
            ...
            try
            {
                demuxer->BuildDeconvBlock(originalSpectrumIndex, muxIndices, masks, signal);
            }
            catch (...)
            {
                cerr << "Unknown error in BuildDeconvBlock on index " << originalSpectrumIndex << endl;
                throw;
            }
            ...
            std::cout << "BuildDeconvBlock: " << duration << endl;
#endif

#ifdef _PROFILE_PERFORMANCE
#ifdef _PROFILE_SOLVER_CONVERGENCE
            auto nnlsSolver = boost::dynamic_pointer_cast<NNLSSolver>(demuxSolver_);
            if (nnlsSolver)
            {
                nnlsSolver->setIterationCaching(true);
            }
#endif
#endif
            ...
            demuxSolver->Solve(masks, signal, solution);

#ifdef _PROFILE_PERFORMANCE
            // add function to be timed here
            t2 = high_resolution_clock::now();
            duration = duration_cast<microseconds >(t2 - t1).count();
            std::cout << "Solve: " << duration << endl;
#endif
            ...
            if (nnlsSolver)
            {
                nnlsSolver->writeIterations(std::cout);
            }
#endif
#endif
    ...
    SpectrumList_Demux::Impl::Impl(const SpectrumListPtr& inner, const Params& p, DataProcessingPtr dp) :
        demuxSolver_(boost::make_shared<NNLSSolver>(p.nnlsMaxIter, p.nnlsEps)),
        params_(p)
#ifdef _USE_DEMUX_DEBUG_WRITER      
        ,
...
#endif
    {
        ...
        sl_ = boost::make_shared<SpectrumListThreadsafe>(sl_);
    ...
    {
        boost::scoped_ptr<lock_guard_semaphore> lock;
        if (threadlimitSemaphore_)
            lock.reset(new lock_guard_semaphore(*threadlimitSemaphore_));
    ...
    Spectrum_const_ptr SpectrumList_Demux::Impl::GetDemuxSpectrum(size_t index) const
    {
        ...
        MatrixPtr solution = cachedSolutions_->GetSolution(request.spectrumOriginalIndex);
